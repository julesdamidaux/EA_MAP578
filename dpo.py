...
def train(model, ref_model, dataset, tokenizer, beta):
    ...
    dpo_config = DPOConfig(
        ...
        output_dir=f'dpo_{beta}_{training_size}',
        logging_steps=4,
        loss_type="sigmoid",
...
small_dataset = load_dataset("Anthropic/hh-rlhf", split="train").select(range(training_size))
...
for beta in betas:
    ...
    train(model, ref_model, small_dataset, tokenizer, beta)
